...
using Teste.RotaViagem.Application.CheapestRouteCalculation.Contracts;
...
namespace Teste.RotaViagem.Application.Travels.Services;
...
internal class TravelService : ITravelService
{
    ...
    public async Task<Result<IEnumerable<Travel>>> GetByDestinationAsync(Location destination)
    {
        ...
        if (query.Count == 0)
        {
            _logger.LogInformation("Encontrado {travelCount} para o destino {destination}", query.Count, destination);
    ...
    public async Task<Result<IEnumerable<Travel>>> GetByStartingPointAsync(Location startingPoint)
    {
        ...
        if (query.Count == 0)
        {
            _logger.LogInformation("Encontrado {travelCount} para ponto de origem {startingPoint}", query.Count, startingPoint);
    ...
    public async Task<Result<string>> AddTravelAsync(string startingPoint, string destination, decimal amount)
    {
        try
        {
            ...
            _logger.LogInformation("Viagem adicionada aos registros - Dados de viagem {travel}", JsonSerializer.Serialize(travel));
            return Result<string>.Success(id);
        }
        catch (Exception ex)
        {
            _logger.LogError("Falha ao adicionar viagem aos registros - Dados de viagem {travel} - Erro {exception}",
    ...
    public async Task<Result> DeleteTravelAsync(string travelId)
    {
        ...
        {
            _logger.LogError("Falha ao excluir viagens dos registros - ID da viagem {travelId} - Erro {exception}", travelId, ex);
